...
class PHPExcel_Calculation
{
    ...
    public function _calculateFormulaValue($formula, $cellID = null, PHPExcel_Cell $pCell = null)
    {
        ...
        $ss = substr($wsTitle,0, 1);
        if (($ss !== "\x00") && ($this->cyclicReferenceStack->onStack($wsCellReference))) {
...
//echo 'Assessing Expression '.substr($formula, $index), PHP_EOL;
            $opCharacter = substr($formula,$index+1, 1);    //    Get the first character of the value at the current index position
//echo 'Initial character of expression block is '.$opCharacter, PHP_EOL;
            if ((isset(self::$comparisonOperators[$opCharacter])) && (strlen($formula) > $index) && (isset(self::$comparisonOperators[substr($formula, $index, 1)]))) {
                $opCharacter .=  substr($formula, (++$index)-1, 1);
...
//                        echo 'Element '.$localeConstant.' is an Excel Constant<br />';
            ...
            //    Ignore white space
            while ((substr($formula,$index+1, 1) == "\n") || (substr($formula,$index+1, 1) == "\r")) {
                ++$index;
            }
            if (substr($formula,$index+1, 1) == ' ') {
                while (substr($formula,$index+1, 1) == ' ') {
                    ++$index;
                }
