...
class PHPExcel_ReferenceHelper
{
    ...
    private function updateSingleCellReference($pCellReference = 'A1', $pBefore = 'A1', $pNumCols = 0, $pNumRows = 0)
    {
        if (strpos($pCellReference, ':') === false && strpos($pCellReference, ',') === false) {
            ...
            $updateColumn = ((substr($newColumn, 0, 1) != '$') && (substr($beforeColumn, 0, 1) != '$') && (PHPExcel_Cell::columnIndexFromString($newColumn) >= PHPExcel_Cell::columnIndexFromString($beforeColumn)));
            $updateRow = ((substr($newRow, 0, 1) != '$') && (substr($beforeRow, 0, 1) != '$') && $newRow >= $beforeRow);
